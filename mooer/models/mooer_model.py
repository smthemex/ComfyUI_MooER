...
from ..utils.utils import print_module_size
from .encoder import WhisperWrappedEncoder, HubertEncoder, W2vBert2Encoder, ParaformerEncoder
...
def init_model(model_config,Qwen2_repo,mooer_repo,encoder_name,mode_choice):
    tokenizer = setup_tokenizer(model_config,Qwen2_repo)
    encoder = setup_encoder(model_config,mooer_repo,encoder_name)
    adapter = setup_adapter(model_config)
    llm = setup_llm(model_config,Qwen2_repo,mooer_repo,mode_choice)
...
def setup_tokenizer(model_config,Qwen2_repo):
...
def setup_encoder(model_config,mooer_repo,encoder_name):
    #encoder_name = model_config.encoder_name
    model_path=os.path.join(mooer_repo,"paraformer_encoder/paraformer-encoder.pth") #暂时只有这个
    if encoder_name == "whisper":
        encoder = WhisperWrappedEncoder.load(model_path)
    elif encoder_name == "hubert":
        encoder = HubertEncoder.load(model_config,model_path)
    elif encoder_name == "w2v_bert2.0":
        encoder = W2vBert2Encoder.load(model_config,model_path)
    elif encoder_name == "paraformer":
        encoder = ParaformerEncoder.load(model_config,model_path)
    else:
...
def setup_llm(model_config,Qwen2_repo,mooer_repo,mode_choice):
    ...
    if os.path.exists(lora_dir):
        if model_config.is_inference:
            ...
            logger.info("loading lora_dir from: {}".format(lora_dir))
            model = PeftModel.from_pretrained(model=model, model_id=lora_dir, is_trainable=False)
            ...
            logger.info("loading lora_dir from: {}".format(lora_dir))
            model = PeftModel.from_pretrained(model=model, model_id=lora_dir, is_trainable=True)
            logger.info("Start Merging LLM and Adaptor...")
            model = model.merge_and_unload()
            logger.info("Finish Merging LLM and Adaptor...")
    return model
